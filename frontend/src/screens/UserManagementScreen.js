...
const UserManagementScreen = ({ navigation, route }) => {
  ...
  const fetchUsers = async () => {
    try {
      setFetching(true);
      const data = await backendUsers.list();
      setUsers(data || []);
    } catch (error) {
      Alert.alert(
        "Error",
  ...
  const handleUpdateUser = async () => {
    try {
      ...
      await backendUsers.update(editingUser.id, updates);
      Alert.alert("Success", "User updated!");
      setEditModalVisible(false);
      fetchUsers();
    } catch (error) {
      Alert.alert("Error", error?.response?.data?.error || "Could not update user.");
    } finally {
      setLoading(false);
    }
  };
      ...
      [
        { text: "Cancel" },
        {
          ...
          onPress: async () => {
            try {
              setLoading(true);
              await backendUsers.remove(editingUser.id);
              setEditModalVisible(false);
              fetchUsers();
            } catch (error) {
              Alert.alert("Error", error?.response?.data?.error || "Could not delete user.");
            } finally {
              setLoading(false);
            }
          },
