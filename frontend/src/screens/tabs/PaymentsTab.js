...
import { backendLedger, backendPayments, backendPurchases, backendUsers } from "../../services/apiClient";
...
const PaymentsTab = ({ user, navigation }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    const getLedgerSafe = async (vendorId, branchName) => {
      try {
        return await backendLedger.getVendorLedger(vendorId, branchName);
      } catch (error) {
    ...
    if (user.role === "Super Admin") {
          ...
          uniqueBranches.map(async (branchName) => {
            ...
            const vendorsWithBalances = await Promise.all(
              branchVendors.map(async (v) => {
                const ledgerRes = await getLedgerSafe(v.id, branchName);
                return {
                  ...v,
                  balance: ledgerRes?.balance || 0,
                };
              }),
            );
      ...
      if (vendors) {
        const vendorsWithBalances = await Promise.all(
          vendors.map(async (v) => {
            const ledgerRes = await getLedgerSafe(v.id, branchName);
            return { ...v, balance: ledgerRes?.balance || 0 };
          }),
        );
      ...
      if (!vendorBranches.length) {
        ...
        const ledgerResults = await Promise.all(
          vendorBranches.map((branch) => getLedgerSafe(user.id, branch)),
        );
        ...
        const combinedBalance = ledgerResults.reduce(
          (sum, res) => sum + (res.balance || 0),
          0,
        );
        const combinedLedger = ledgerResults
  ...
  const saveComment = async () => {
    ...
    const commentPayload = commentText.trim();

    try {
      if (activeLedgerItem.ledgerType === "Purchase") {
        await backendPurchases.updateVendorComment(activeLedgerItem.id, commentPayload);
      } else {
        await backendPayments.updateVendorTransactionComment(
          activeLedgerItem.id,
          commentPayload,
        );
      }
      ...
      setMyLedger((prev) =>
        prev.map((entry) =>
          ...
          entry.ledgerType === activeLedgerItem.ledgerType
            ? { ...entry, vendor_comment: commentPayload }
            : entry,
        ),
      );
      closeCommentModal();
